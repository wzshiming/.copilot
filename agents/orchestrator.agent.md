...
2. **Execute loop** — Per stage, dispatch a _Coder_ subagent. Subagents are stateless: every dispatch must be self-contained (overall goal, stage scope, acceptance criteria, relevant files, summary of prior stage outputs) and states that the Orchestrator reviews the result, so the Coder skips its own Reviewer pass. Do small fixes yourself; delegate anything substantial. Stages that touch disjoint files may run as parallel _Coder_ dispatches, each in its own worktree: create the worktrees yourself, one at a time, with the git-worktree skill before dispatching; pass the absolute worktree path and branch to each Coder and to every review dispatch for that stage; record both in the ledger; land each branch with finish-branch before removing its worktree.
