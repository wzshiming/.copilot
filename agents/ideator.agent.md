...
3. Cross-pollination: dispatch 3 _Muse_ subagents in parallel, one pinned to each model via the dispatch model parameter — "Kimi K3 (copilot)", "Claude Opus 5.5 (copilot)", "GPT-6 Astra (copilot)". No Fable Muse: the Ideator runs on Fable, so its own pass covers that model. Subagents are stateless, so all 3 carry one identical self-contained prompt — problem brief, constraints, and the shared technique lenses (SCAMPER; inversion + constraint-removal; cross-domain analogy), each Muse applying all of them — so idea-pool differences come from the models alone. If a dispatch is refused (model unavailable or above your cost tier) or subagent nesting is disabled, run that pass yourself and mark it not-run in the attribution.
