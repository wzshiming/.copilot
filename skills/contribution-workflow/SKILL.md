...
| PR                | [pr.md](./references/pr.md)         | Duplicate PR search, filling the PR template (`/kind`, `release-note`, checklists), cross-fork `gh pr create`, `WIP:` title for unfinished PRs, `Fixes #N` linking, CI watch-and-fix loop (`gh pr checks --watch`), verification, final tone pass |
